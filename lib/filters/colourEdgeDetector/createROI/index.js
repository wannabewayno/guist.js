const checkPeriodic = require('../checkPeriodic');
const { Matrix } = require('ml-matrix');

module.exports = function( pixels, i, j, rows, columns) {
    ...
    for (let ii = 0; ii < rows; ii++) {
        for (let jj = 0; jj < columns; jj++) {
            const row = checkPeriodic(minX+ii,0, pixels.rows - 1);
            const column = checkPeriodic(minY+jj,0, pixels.columns - 1);
